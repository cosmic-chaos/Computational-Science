...
namespace GraphControl
{
    abstract public class TransformingObject
    {
        ...
        public void UpdateMatrix(double width, double height, double widthOffset, double heightOffset, Range range)
        {
            Matrix mat = Transform.Matrix;
            double ax = width / (range.MaxX - range.MinX);
            double ay = height / (range.MinY - range.MaxY);
            mat.M11 = ax;
            mat.M22 = ay;
            mat.OffsetX = -ax * range.MinX + widthOffset;
            mat.OffsetY = -ay * range.MaxY + heightOffset;
        ...
        protected void UpdateAxes(double width, double height, double widthOffset, double heightOffset, Range range)
        {
            ...
            xAxis.Update(width, heightOffset, range.MinX, range.MaxX);
            yAxis.Update(height, widthOffset, range.MinY, range.MaxY);
        }
    }
}
